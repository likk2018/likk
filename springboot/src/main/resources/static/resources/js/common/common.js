//var DOMIN = {'MAIN':'https://wx.taoxiaoguai.cn/mini'};
var DOMIN = { 'MAIN': 'http://127.0.0.1:8080/mini' };
//var DOMIN = {'MAIN':'https://wx.taoxiaoguai.cn/mini-www'};
$.noDataFond = '没有查询到数据。';
...
(function (doc, win) {
	var docEl = doc.documentElement,
		...
		recalc = function () {
			...
			if(clientWidth / clientHeight < 0.5){
...
function showToast(content) {
	let toastEl = `
						<div id="toast" style="position:fixed;top:50%;left:50%;transform:translateX(-50%) translateY(-50%);min-width:130px;text-align:center;color:#FCFEFE;background:rgba(76,90,90,.7);padding:10px 60px;border-radius:30px;">
							${content}
						</div>
					 `
	var obj = $(toastEl).appendTo(document.body);
	setTimeout(function(){
